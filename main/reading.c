...
reading take_reading(){
    ...
    init_heartrate();   //initialize max30120
        ...
        xTaskCreate(take_oxy_reading, "oxygen_reading", 4096, NULL, 3, NULL);
        while(!isTaskCompleted || !temp_reading_complete){
            if(prev_finger_state != finger_not_placed){
                if(!finger_not_placed){
                    ...
                    xTaskCreate(take_temperature, "temperature_reading", 4096, NULL, 2, &temperature_task);
                }
...
void app_main(void)
{
    ...
    while (true) {
    	...
    	printf(scan_result_data,"%d,%3.2f,%3.2f,%3.2f,%3.2f",sensor_readings.heartrate,sensor_readings.heart_precision,sensor_readings.oxygenLevel,sensor_readings.oxy_precision,sensor_readings.temperature);
